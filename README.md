...
1. **先看插件是否能在终端输出**：
   ...
   或用 venv Python 测试（结果应与上一条一致）：

   ```bash
   ~/.swiftbar-venv/bin/python3 "$(defaults read com.ameba.SwiftBar PluginDirectory 2>/dev/null || echo "$HOME/.swiftbar-plugins")/tokens.3m.py"
   ```
...
# 2. 确保 shebang 指向 venv Python（install.sh 已创建 ~/.swiftbar-venv）
